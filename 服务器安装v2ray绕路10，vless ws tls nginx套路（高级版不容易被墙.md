...
#### 修改nginx.conf的基本配置
注意nginx.conf文件在/etc/nginx/conf
注意打开路径下所有文件包的777权限
...
粘贴代码进default.conf 文件里（原来的，现在不用了）
```
server {
...
浏览器打开fsfsfs.eu.org查看会不会跳转到nginx网站了，会跳转说明nginx就成功了，虽然nginx的active状态还是红色，但是不影响

打开fsfsfs.eu.org/ray网站，如果返回bad request就说明8443端口连接到v2ray了
如果出现502，则说明8443端口的v2ray连接不成功
...
# 或者windows的Qvray客户端下载和配置套路（现在不用了）
- 1.win平台：
...
发现是服务器时间与当前系统时间不一致引起。 由于服务器是[虚拟机](https://so.csdn.net/so/search?q=%E8%99%9A%E6%8B%9F%E6%9C%BA&spm=1001.2101.3001.7020)，导致时间不一致。
